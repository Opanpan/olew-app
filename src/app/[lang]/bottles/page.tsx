...
function BottlesCatalog() {
  ...
  // Parse attribute filters from URL: attr_material=PET,HDPE
  const activeAttrs = (() => {
  ...
  // Parse range filters from URL: range_diameter=35,101
  const activeRanges = (() => {
  ...
  useEffect(() => {
    async function loadFilters() {
      ...
      setFilterData(data);
      if (data) {
        const t = data.types.find((x) => x.name_en.toLowerCase() === 'bottle');
        setTypeId(t?.id);
      }
  ...
  // Attribute filtering — applied after API fetch.
  // ProductListItem doesn't carry attribute data so we filter by name match as best-effort.
  // Category + search are the reliable API-level filters.
  const attrFilteredProducts = allProducts.filter((p) => {
    ...
    // Keep product if its name contains at least one value from each active filter key
